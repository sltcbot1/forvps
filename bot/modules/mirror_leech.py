...
from bot import dispatcher, DOWNLOAD_DIR, LOGGER, MEGA_API_KEY
...
def _mirror_leech(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False):
    ...
    if is_gdrive_link(link):
        ...
        if MEGA_API_KEY is not None:
            Thread(target=MegaDownloader(listener).add_download, args=(link, f'{DOWNLOAD_DIR}{listener.uid}/', listener, name)).start()
        else:
